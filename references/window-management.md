...
> 下列坐标基于「双屏 2×1920×1200、Dock 底部自动隐藏」这一特定环境，是**示例模板、不是通用常量**。单屏或不同分辨率/排列时，先用 `system_profiler SPDisplaysDataType` 实测分辨率、用下文「列出所有窗口」读真实坐标，按比例重算，勿照抄 x/y 像素。

- **显示器（示例环境）**：2× 1920×1200（左屏 x=0-1920，右屏 x=1920-3840）
- **Dock**：底部，自动隐藏，tilesize 72，预留 ~90px（底部边界 y=1110）
- **菜单栏**：25px 高（顶部边界 y=25）
- **窗口管理工具**：Spectacle（快捷键是通用能力；是否已安装以本机为准）
